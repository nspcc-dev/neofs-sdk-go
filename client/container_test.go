...
func (x testRFC6979DataSignatureServerSettings[SIGNED, SIGNEDV2, SIGNEDV2PTR]) verifyCommon(signedField string, m *protorefs.SignatureRFC6979) error {
		...
		*/
		t.Run("requests", func(t *testing.T) {
			t.Run("required data", func(t *testing.T) {
				...
				srv.authenticateRequest(c.prm.signer) // TODO: why not parameterized signer?
	...
	t.Run("messages", func(t *testing.T) {
		...
		*/
		...
		t.Run("responses", func(t *testing.T) {
			t.Run("valid", func(t *testing.T) {
				t.Run("payloads", func(t *testing.T) {
					// TODO: add tests asserting result when some optional field is unset (for other methods too)
					for _, tc := range []struct {
						name string
			...
			t.Run("invalid", func(t *testing.T) {
				...
				t.Run("payloads", func(t *testing.T) {
					...
					for _, tc := range []struct {
						...
						{name: "attributes/empty value", msg: "empty attribute value k2", // TODO: message is strange
					...
					for _, tc := range []struct {
						...
						{name: "selectors/clause/negative", msg: "invalid selector #1: negative clause", corrupt: func(valid *protonetmap.PlacementPolicy) {
							valid.Selectors[1].Clause = -1
						}},
						{name: "filters/op/negative", msg: "invalid filter #1: negative op", corrupt: func(valid *protonetmap.PlacementPolicy) {
...
func TestClient_ContainerDelete(t *testing.T) {
	...
	t.Run("sign ID failure", func(t *testing.T) {
		...
		t.Run("wrong scheme", func(t *testing.T) {
			...
			// TODO: currently unchecked and request attempt is done. Better to pre-check like Put does
			...
			t.Run("invalid", func(t *testing.T) {
				...
				t.Run("payloads", func(t *testing.T) {
					...
					for _, tc := range []struct {
						...
						{name: "op/negative", msg: "negative op", corrupt: func(valid *protoacl.EACLRecord) {
							valid.Operation = -1
						}},
						{name: "action/negative", msg: "negative action", corrupt: func(valid *protoacl.EACLRecord) {
							valid.Action = -1
						}},
						{name: "filters/header type/negative", msg: "invalid filter #1: negative header type", corrupt: func(valid *protoacl.EACLRecord) {
							valid.Filters = []*protoacl.EACLRecord_Filter{{}, {HeaderType: -1}}
						}},
						{name: "filters/matcher/negative", msg: "invalid filter #1: negative matcher", corrupt: func(valid *protoacl.EACLRecord) {
							valid.Filters = []*protoacl.EACLRecord_Filter{{}, {MatchType: -1}}
						}},
						{name: "targets/role/negative", msg: "invalid target #1: negative role", corrupt: func(valid *protoacl.EACLRecord) {
