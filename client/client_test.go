...
loop:
	...
	dialSrv.signResponsesBy(testServerSignerOnDial)
...
var (
	...
	invalidUserIDProtoTestcases = []struct {
		...
		// TODO: would be better to see user.ErrZero in this case
...
type testCommonUnaryServerSettings[
	...
	testCommonResponseServerSettings[RESPBODY, RESP, RESPV2, RESPV2PTR]
}
...
type testCommonServerStreamServerSettings[
	...
	testCommonServerSettings
	testCommonRequestServerSettings[REQBODY, REQBODYV2, REQBODYV2PTR, REQ, REQV2, REQV2PTR]
	resps    map[uint]testCommonResponseServerSettings[RESPBODY, RESP, RESPV2, RESPV2PTR]
	respErrN uint
	respErr  error
}

// tunes processing of N-th response starting from 0.
func (x *testCommonServerStreamServerSettings[_, _, _, _, _, _, RESPBODY, RESP, RESPV2, RESPV2PTR]) tuneNResp(n uint,
	tune func(*testCommonResponseServerSettings[RESPBODY, RESP, RESPV2, RESPV2PTR])) {
	type t = testCommonResponseServerSettings[RESPBODY, RESP, RESPV2, RESPV2PTR]
	if x.resps == nil {
...
func (x *testCommonServerStreamServerSettings[_, _, _, _, _, _, RESPBODY, RESP, RESPV2, RESPV2PTR]) respondWithoutSigning(n uint) {
	x.tuneNResp(n, func(s *testCommonResponseServerSettings[RESPBODY, RESP, RESPV2, RESPV2PTR]) {
...
func (x *testCommonServerStreamServerSettings[_, _, _, _, _, _, RESPBODY, RESP, RESPV2, RESPV2PTR]) signResponsesBy(n uint, signer neofscrypto.Signer) {
	x.tuneNResp(n, func(s *testCommonResponseServerSettings[RESPBODY, RESP, RESPV2, RESPV2PTR]) {
...
func (x *testCommonServerStreamServerSettings[_, _, _, _, _, _, RESPBODY, RESP, RESPV2, RESPV2PTR]) respondWithMeta(n uint, meta *protosession.ResponseMetaHeader) {
	x.tuneNResp(n, func(s *testCommonResponseServerSettings[RESPBODY, RESP, RESPV2, RESPV2PTR]) {
...
func (x *testCommonServerStreamServerSettings[_, _, _, _, _, _, RESPBODY, RESP, RESPV2, RESPV2PTR]) respondWithStatus(n uint, st *protostatus.Status) {
	x.tuneNResp(n, func(s *testCommonResponseServerSettings[RESPBODY, RESP, RESPV2, RESPV2PTR]) {
...
func (x *testCommonServerStreamServerSettings[_, _, _, _, _, _, RESPBODY, RESP, RESPV2, RESPV2PTR]) respondWithBody(n uint, body RESPBODY) {
	x.tuneNResp(n, func(s *testCommonResponseServerSettings[RESPBODY, RESP, RESPV2, RESPV2PTR]) {
...
//
// Overrides respondWithStatus.
func (x *testCommonServerStreamServerSettings[_, _, _, _, _, _, _, _, _, _]) abortHandlerAfterResponse(n uint, err error) {
	if n == 0 {
...
func (x *testCommonClientStreamServerSettings[_, _, _, _, _, _, _, _, _, _]) abortHandlerAfterRequest(n uint, err error) {
	if n == 0 {
		x.setHandlerError(err)
	} else {
...
// received.
func (x *testCommonClientStreamServerSettings[_, _, _, _, _, _, _, _, _, _]) respondAfterRequest(n uint) {
	x.respN = n
}
...
//
// Overrides signResponsesBy.
func (x *testCommonResponseServerSettings[_, _, _, _]) respondWithoutSigning() {
	x.respUnsigned = true
}
...
//
// No-op if signing is disabled using respondWithoutSigning.
func (x *testCommonResponseServerSettings[_, _, _, _]) signResponsesBy(signer neofscrypto.Signer) {
	x.respSigner = signer
}
...
//
// Overrides respondWithStatus.
func (x *testCommonResponseServerSettings[_, _, _, _]) respondWithMeta(meta *protosession.ResponseMetaHeader) {
	x.respMeta = meta
}
...
//
// Overrides respondWithMeta.
func (x *testCommonResponseServerSettings[_, _, _, _]) respondWithStatus(st *protostatus.Status) {
...
// body is returned.
func (x *testCommonResponseServerSettings[RESPBODY, _, _, _]) respondWithBody(body RESPBODY) {
	x.respBody = proto.Clone(body).(RESPBODY)
	x.respBodyForced = true
}

func (x testCommonResponseServerSettings[_, RESP, RESPV2, RESPV2PTR]) signResponse(resp *RESP) (*RESP, error) {
	if x.respUnsigned {
		return resp, nil
	}
	var r RESPV2
...
func testInvalidResponseVerificationHeader[SRV interface {
...
func testStatistic[SRV interface {
	...
	for _, getNonStatErr := range customNonStatFailures {
		err := getNonStatErr(c)
		require.Error(t, err)
		// TODO: strange that stats of such errors are similar to OK
		assert(nil)
		collected = nil
	}
