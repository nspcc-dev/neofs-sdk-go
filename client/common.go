...
func dowithTimeout(desc string, timeout time.Duration, cancel context.CancelFunc, action func() error) error {
	st := time.Now()
	uid := uuid.New()
	fmt.Fprintf(os.Stderr, "TESTLOG start op %s ID %s timeout %s\n", desc, uid, timeout)
	ch := make(chan error, 1)
	go func() {
		err := action()
		fmt.Fprintf(os.Stderr, "TESTLOG fin op %s ID %s took %s\n", desc, uid, time.Since(st))
		ch <- err
		close(ch)
	}()
	...
	select {
	case err := <-ch:
		fmt.Fprintf(os.Stderr, "TESTLOG return op %s ID %s took %s\n", desc, uid, time.Since(st))
		return err
	case <-tt.C:
		fmt.Fprintf(os.Stderr, "TESTLOG deadline op %s ID %s took %s", desc, uid, time.Since(st))
		cancel()
