...
// Cross-service.
var (
	// set of correct user IDs.
	validProtoUserIDs = []*protorefs.OwnerID{
		{Value: []byte{53, 28, 187, 10, 189, 59, 106, 227, 123, 6, 44, 200, 77, 80, 253, 72, 33, 96, 198, 185, 219, 239, 171, 67, 127}},
		{Value: []byte{53, 161, 152, 17, 205, 254, 2, 98, 40, 148, 36, 174, 244, 176, 247, 233, 240, 190, 173, 56, 130, 127, 109, 227, 193}},
	}
...
var (
	...
	validFullPutContainerResponseBody = proto.Clone(validMinPutContainerResponseBody).(*protocontainer.PutResponse_Body)
	...
	validMinProtoNetInfo = &protonetmap.NetworkInfo{
		NetworkConfig: &protonetmap.NetworkConfig{
			Parameters: []*protonetmap.NetworkConfig_Parameter{
				{Value: []byte("any")}, // TODO: empty key is OK?
			},
		},
	}
		...
		SessionToken: &protosession.SessionToken{
			Body: &protosession.SessionToken_Body{
				...
				Context: &protosession.SessionToken_Body_Object{
					Object: &protosession.ObjectSessionContext{Verb: 1849442930, Target: &protosession.ObjectSessionContext_Target{
...
func checkContainerSessionTransport(t session.Container, m *protosession.SessionToken) error {
	c, err := checkCommonSessionTransport(&t, m) // TODO: strange that verification requires a pointer
	if err != nil {
		return err
	}
...
func checkStoragePolicyTransport(p netmap.PlacementPolicy, m *protonetmap.PlacementPolicy) error {
	...
	for i, cs := range css {
		...
		switch {
		default:
			// TODO: better to define enum and expose the value
...
func checkContainerTransport(c container.Container, m *protocontainer.Container) error {
	...
	for _, ma := range m.GetAttributes() {
		...
		if tail, ok := strings.CutPrefix(k, "__NEOFS__"); ok {
			...
			usrAttrs = append(usrAttrs, [2]string{k, v})
		}
	}
	var cas [][2]string
...
func checkNetInfoTransport(n netmap.NetworkInfo, m *protonetmap.NetworkInfo) error {
	...
	var cps [][2]any
	n.IterateRawNetworkParameters(func(name string, value []byte) { cps = append(cps, [2]any{name, value}) })
	if v1, v2 := len(cps), len(mps); v1 != v2 {
		return fmt.Errorf("number of config parameters (client: %d, message: %d)", v1, v2)
	}
