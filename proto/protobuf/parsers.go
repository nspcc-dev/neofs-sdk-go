...
// SkipField skips following field with preread number and type.
//
// If no error, positions x right after the field.
//
// If there is an error, its text contains num and typ.
func (x *BuffersSlice) SkipField(num protowire.Number, typ protowire.Type) error {
	switch typ {
	case protowire.VarintType:
		_, err := x.ParseUint64Field(num, typ)
		return err
	case protowire.Fixed64Type:
		_, ok := x.MoveNext(fixed64Len)
		if !ok {
			return wrapParseFieldError(num, typ, newTruncatedBufferError(fixed64Len, 0))
		}
		return nil
	case protowire.BytesType:
		_, err := x.ParseLENField(num, typ)
		return err
	case protowire.StartGroupType, protowire.EndGroupType:
		return wrapParseFieldError(num, typ, errors.New("type is not supported"))
	case protowire.Fixed32Type:
		_, ok := x.MoveNext(fixed32Len)
		if !ok {
			return wrapParseFieldError(num, typ, newTruncatedBufferError(fixed32Len, 0))
		}
		return nil
	default:
		return newUnknownFieldTypeError(typ)
	}
}
