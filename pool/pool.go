...
func newClientStatusMonitor(addr string, errorThreshold uint32, thresholdWindowSize time.Duration) clientStatusMonitor {
	m := clientStatusMonitor{
		...
		healthy:           &atomic.Bool{},
		overallErrorCount: &atomic.Uint64{},
		sw:                newSlidingWindow(thresholdWindowSize, uint64(errorThreshold)),
	}
...
type (
	slidingWindow struct {
		...
		windowStart *atomic.Uint64 // Nanoseconds

		rotationMu *sync.Mutex
	}
)

func newSlidingWindow(windowSize time.Duration, limit uint64) *slidingWindow {
	var windowStart atomic.Uint64
	windowStart.Store(uint64(time.Now().UnixNano()))
...
func (sw *slidingWindow) advance(now uint64) {
	start := sw.windowStart.Load()
...
func (sw *slidingWindow) Current() uint64 {
