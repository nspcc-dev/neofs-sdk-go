...
//
// Returns any network config parsing errors.
func (c Container) AssertNetworkConfig(cfg netmap.NetworkConfig) (bool, error) {
	v, err := homomorphicHashDisabled(cfg)
	if err != nil {
		return false, nil
	}
	...
	// network config contains
	// reverse (disabling) setting
	return c.HomomorphicHashingState() != v, nil
}

const HomomorphicHashingDisabledKey = "HomomorphicHashingState"
