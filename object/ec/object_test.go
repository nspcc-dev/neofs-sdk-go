...
func TestGetPartInfo(t *testing.T) {
	...
	pi, err := ec.GetPartInfo(obj)
	require.NoError(t, err)
	require.Equal(t, ec.PartInfo{RuleIndex: 123, Index: 456}, pi)
}
...
func TestFormObjectForECPart(t *testing.T) {
	...
	partInfo := ec.PartInfo{RuleIndex: 123, Index: 456}
...
func TestDecodePartInfoFromAttributes(t *testing.T) {
	t.Run("missing", func(t *testing.T) {
		pi, err := ec.DecodePartInfoFromAttributes("", "")
		require.NoError(t, err)
		require.EqualValues(t, -1, pi.RuleIndex)
	})
	...
	t.Run("failure", func(t *testing.T) {
		for _, tc := range []struct {
			...
			t.Run(tc.name, func(t *testing.T) {
				_, err := ec.DecodePartInfoFromAttributes(tc.ruleIdx, tc.partIdx)
				tc.assertErr(t, err)
			})
		}
	})

	pi, err := ec.DecodePartInfoFromAttributes("12", "34")
	require.NoError(t, err)
	require.Equal(t, ec.PartInfo{RuleIndex: 12, Index: 34}, pi)
}

func TestObjectWithAttributes(t *testing.T) {
	var obj object.Object
	require.False(t, ec.ObjectWithAttributes(obj))
	...
	for _, attr := range []string{
		...
		require.True(t, ec.ObjectWithAttributes(obj), attr)
	}
}
