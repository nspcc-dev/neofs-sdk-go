...
func TestNodeSequenceForPartReading(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(fmt.Sprintf("part=%d,parts=%d,nodes=%d", tc.partIdx, tc.totalParts, tc.nodes), func(t *testing.T) {
			require.Equal(t, tc.exp, slices.Collect(ec.NodeSequenceForPart(tc.partIdx, tc.totalParts, tc.nodes)))

			var collected []int
			for i := range ec.NodeSequenceForPart(tc.partIdx, tc.totalParts, tc.nodes) {
