...
func ExampleCalculateDataSignature() {
	// instantiate signer
	...
	for _, signer := range []neofscrypto.Signer{
		...
		// calculate signature
		sig, _ := neofscrypto.CalculateDataSignature(signer, data)
...
func ExampleIsValidDataSignature() {
	...
	for _, tc := range []struct {
		...
		valid := neofscrypto.IsValidDataSignature(sig, data)
