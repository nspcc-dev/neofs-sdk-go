...
func TestNewNonce(t *testing.T) {
	t.Run("generates unique values", func(t *testing.T) {
		...
		require.Greater(t, len(nonces), iterations*95/100)
	})
}
...
func TestToken_ValidateFields(t *testing.T) {
	for _, tc := range []struct {
		...
		{"wrong version", "invalid fields: invalid token version: expected 0, got 2", func(tok *session.Token) {
			tok.SetVersion(2)
		}},
		{"missing issuer", "invalid fields: issuer is not set", func(tok *session.Token) {
			tok.SetIssuer(user.ID{})
		}},
		{"no subjects", "invalid fields: no subjects specified", func(tok *session.Token) {
			require.NoError(t, tok.SetSubjects([]session.Target{}))
		}},
		{"empty subject", "invalid fields: subject at index 1 is empty", func(tok *session.Token) {
			require.NoError(t, tok.AddSubject(session.Target{}))
		}},
		{"missing iat", "invalid fields: issued at (iat) is not set", func(tok *session.Token) {
			tok.SetIat(0)
		}},
		{"missing nbf", "invalid fields: not before (nbf) is not set", func(tok *session.Token) {
			tok.SetNbf(0)
		}},
		{"missing exp", "invalid fields: expiration (exp) is not set", func(tok *session.Token) {
			tok.SetExp(0)
		}},
		{"nbf after exp", "invalid fields: not before (nbf) is after expiration (exp)", func(tok *session.Token) {
			tok.SetNbf(300)
			tok.SetExp(200)
		}},
		{"iat after exp", "invalid fields: issued at (iat) is after expiration (exp)", func(tok *session.Token) {
			tok.SetIat(400)
		}},
		{"no contexts", "invalid fields: no contexts specified", func(tok *session.Token) {
			require.NoError(t, tok.SetContexts([]session.Context{}))
		}},
		{"empty context", "invalid fields: context at index 0 has no verbs", func(tok *session.Token) {
			require.NoError(t, tok.SetContexts([]session.Context{{}}))
		}},
		{"unsigned token", "invalid fields: token is not signed", func(tok *session.Token) {
			// Don't sign the token
		}},
		{"invalid delegation", "invalid fields: token is not signed", func(tok *session.Token) {
			del := newValidToken(t)
			tok.SetOrigin(&del)
		}},
	} {
	...
	t.Run("invalid signature", func(t *testing.T) {
		...
		err := tok.Validate()
		require.EqualError(t, err, "invalid fields: token signature verification failed")
	})
...
func TestToken_ValidateDelegationChain(t *testing.T) {
	...
	t.Run("invalid fields", func(t *testing.T) {
		...
		t.Run("origin token has invalid lifetime", func(t *testing.T) {
			...
			err := tok.Validate()
			require.EqualError(t, err, "depth 1: invalid fields: not before (nbf) is not set")
		})
	...
	t.Run("lifetime narrowing", func(t *testing.T) {
		t.Run("delegation nbf before token nbf", func(t *testing.T) {
			...
			tok := newTokenForDelegation(t, subject.UserID(), session.NewTargetUser(usertest.ID()))
			tok.SetNbf(50) // origin.Nbf() > tok.Nbf()
			tok.SetExp(300)
			tok.SetOrigin(&origin)
			require.NoError(t, tok.Sign(subject))

			err := tok.Validate()
			require.EqualError(t, err, "origin token lifetime is outside this token's lifetime")
		})
		...
		t.Run("delegation timestamp outside token lifetime", func(t *testing.T) {
			...
			tok := newTokenForDelegation(t, subject.UserID(), session.NewTargetUser(usertest.ID()))
			tok.SetNbf(200)
			tok.SetExp(600) // origin.Exp() < tok.Exp()
			tok.SetOrigin(&origin)
			require.NoError(t, tok.Sign(subject))
		...
		t.Run("valid exact lifetime match", func(t *testing.T) {
			...
			tok := newTokenForDelegation(t, subject.UserID(), session.NewTargetUser(usertest.ID()))
			tok.SetNbf(200) // Same nbf
			tok.SetExp(300) // Same exp
			tok.SetOrigin(&origin)
			require.NoError(t, tok.Sign(subject))
		...
		t.Run("valid nested lifetime", func(t *testing.T) {
			...
			tok := newTokenForDelegation(t, subject.UserID(), session.NewTargetUser(usertest.ID()))
			tok.SetNbf(225) // Later nbf
			tok.SetExp(275) // Earlier exp
			tok.SetOrigin(&origin)
			require.NoError(t, tok.Sign(subject))
	...
	t.Run("context narrowing", func(t *testing.T) {
		t.Run("delegation with unauthorized verb", func(t *testing.T) {
			...
			err = tok.Validate()
			require.EqualError(t, err, "invalid origin chain: context at index 0 uses verbs not authorized by origin token")
		})
	...
	t.Run("issuer authorization", func(t *testing.T) {
		t.Run("issuer not in origin token subjects", func(t *testing.T) {
			...
			err := tok.Validate()
			require.EqualError(t, err, "token issuer is not in this origin token's subjects")
		})
		...
		t.Run("issuer in origin token NNS subjects", func(t *testing.T) {
			...
			hasUser := func(name string, userId user.ID) bool {
				return name == "some.nns" && userId == subject.UserID()
			}
			tok := newTokenForDelegation(t, subject.UserID(), session.NewTargetUser(usertest.ID()))
			tok.SetOrigin(&origin)
			require.NoError(t, tok.Sign(subject))

			err := tok.ValidateWithNNS(hasUser)
			require.NoError(t, err)
		})
	})
	...
	t.Run("cycle detection", func(t *testing.T) {
		t.Run("direct cycle - token references itself", func(t *testing.T) {
			...
			err := tok.Validate()
			require.EqualError(t, err, "cycle detected in delegation chain")
		})
...
func TestToken_AssertAuthority(t *testing.T) {
	...
	require.True(t, tok.AssertAuthority(subject1.UserID(), nil))
	require.True(t, tok.AssertAuthority(subject2, nil))
	require.False(t, tok.AssertAuthority(subject3, nil))
	...
	t.Run("with delegation chain", func(t *testing.T) {
		...
		require.False(t, del.AssertAuthority(subject1.UserID(), nil))
		require.False(t, del.AssertAuthority(subject2, nil))
		require.True(t, del.AssertAuthority(subject3, nil)) // direct subject
	})

	t.Run("error cases", func(t *testing.T) {
		t.Run("empty subjects", func(t *testing.T) {
			var emptyTok session.Token
			require.False(t, emptyTok.AssertAuthority(subject2, nil))
		})

		t.Run("zero user", func(t *testing.T) {
			require.False(t, tok.AssertAuthority(user.ID{}, nil))
		})
	})

	t.Run("AssertAuthority with NNS", func(t *testing.T) {
		t.Run("direct user id match", func(t *testing.T) {
			user1 := usertest.User()
			user2 := usertest.ID()
			user3 := usertest.ID()

			tok := newTokenForDelegation(t, user1.UserID(), session.NewTargetUser(user2))
			require.NoError(t, tok.Sign(user1))

			hasUser := func(name string, userID user.ID) bool {
				return false // no NNS mapping
			}

			require.True(t, tok.AssertAuthority(user2, hasUser))
			require.False(t, tok.AssertAuthority(user3, hasUser))
		})
		...
		t.Run("nns subject with user mapping", func(t *testing.T) {
			...
			hasUser := func(name string, userID user.ID) bool {
				return name == "example.neo" && userID == mappedUser
			}

			require.True(t, tok.AssertAuthority(mappedUser, hasUser))
			require.False(t, tok.AssertAuthority(otherUser, hasUser))
		})
		...
		t.Run("multiple nns subjects", func(t *testing.T) {
			...
			require.True(t, tok.AssertAuthority(user1, hasUser))
			require.True(t, tok.AssertAuthority(user2, hasUser))
			require.False(t, tok.AssertAuthority(user3, hasUser))
		})

		t.Run("nns and user id subjects mixed", func(t *testing.T) {
			issuer := usertest.User()
			directUser := usertest.ID()
			mappedUser := usertest.ID()
			otherUser := usertest.ID()

			tok := newTokenForDelegation(t, issuer.UserID(), session.NewTargetUser(directUser))
			require.NoError(t, tok.AddSubject(session.NewTargetNamed("example.neo")))
			require.NoError(t, tok.Sign(issuer))
			...
			require.True(t, tok.AssertAuthority(directUser, hasUser))
			// NNS mapped user
			require.True(t, tok.AssertAuthority(mappedUser, hasUser))
			// Not authorized
			require.False(t, tok.AssertAuthority(otherUser, hasUser))
		...
		t.Run("empty subjects", func(t *testing.T) {
			...
			require.False(t, tok.AssertAuthority(usertest.ID(), hasUser))
		})
	})
}
